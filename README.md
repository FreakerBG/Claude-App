# Momentum — Personal OS

A private, installable **PWA** (progressive web app) designed as a personal
command center. It brings **tasks**, **money**, **habits**, **goals**, and a
**journal** into one focused system. Momentum installs on **iPhone and PC**,
works **offline**, and stores everything on your device (with a portable backup).

The v3 interface uses a responsive command-center visual system: HUD-style
progress, glass telemetry panels, line icons, ambient scan effects, route
transitions, and motion-reduction support for accessibility.
...
2. Click **Install app** inside the desktop sidebar. If it is not shown, use the
   **Install** icon in the address bar (or menu → *Install Momentum*).
...
Apple meta tags), and `src/components/Nav.tsx` (sidebar brand). The reactor icon
is generated by `scripts/generate-icons.mjs`; run `npm run icons` after changing
its artwork.
